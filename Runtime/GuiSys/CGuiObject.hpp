...
protected:
  zeus::CTransform4f m_initLocalXF;
  zeus::CTransform4f x4_localXF;
  zeus::CTransform4f x34_worldXF;
...
public:
  ...
  const zeus::CTransform4f& GetInitialLocalTransform() const { return m_initLocalXF; }
  const zeus::CVector3f& GetLocalPosition() const { return x4_localXF.origin; }
  const zeus::CTransform4f& GetLocalTransform() const { return x4_localXF; }
  void SetLocalPosition(const zeus::CVector3f& pos);
  const zeus::CVector3f& GetWorldPosition() const { return x34_worldXF.origin; }
  const zeus::CTransform4f& GetWorldTransform() const { return x34_worldXF; }
  void RotateReset();
  ...
  void MultiplyO2P(const zeus::CTransform4f& xf);
  ...
  void SetO2WTransform(const zeus::CTransform4f& xf);
  void SetLocalTransform(const zeus::CTransform4f& xf);
};
