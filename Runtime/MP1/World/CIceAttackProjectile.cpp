...
CIceAttackProjectile::CIceAttackProjectile(const TToken<CGenDescription>& gen1, const TToken<CGenDescription>& gen2,
                                           ...
                                           TUniqueId owner, bool active, const zeus::CTransform4f& xf,
...
, x18c_(unkInt2) {
  ...
  zeus::CVector3f m0 = zeus::CVector3f{0.f, 1.f, 0.f}.cross(xf.GetForward()).normalized();
  zeus::CVector3f m1 = m0.cross(zeus::CVector3f{0.f, 0.f, 1.f}).normalized();
  SetTransform(zeus::CTransform4f(m0, m1, zeus::CVector3f{0.f, 0.f, 1.f}, GetTranslation()));
...
void CIceAttackProjectile::AddToRenderer(const zeus::CFrustumPlanes& frustum, CStateManager& mgr) {
  mgr.AddDrawableActor(*this, -1000.f * CGraphics::mViewMatrix.GetForward(), x9c_renderBounds);
}
