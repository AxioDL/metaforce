...
{
    if (!x320_suitModel || !x320_suitModel.IsLoaded() ||
        !x324_suitSkin || !x324_suitSkin.IsLoaded())
        return;
...
{
    if (x310_loadedCharIdx != x2e8_suitRes.GetCharacterNodeId() ||
        !x64_modelData || !x64_modelData->HasAnimData())
    {
        x2e8_suitRes.SetCharacterNodeId(x310_loadedCharIdx);
...
void CScriptPlayerActor::Think(float dt, CStateManager& mgr)
{
    ...
    if (x354_30_enableLoading)
    {
        ...
        if (!x354_29_loading)
        {
            if (x354_28_suitModelLoading || x354_27_beamModelLoading || !x64_modelData ||
                x64_modelData->IsNull() || !x64_modelData->IsLoaded(0))
                x354_29_loading = true;
        }

        if (x354_29_loading && !x354_28_suitModelLoading && !x354_27_beamModelLoading &&
            x64_modelData && !x64_modelData->IsNull() && x64_modelData->IsLoaded(0))
        {
...
void CScriptPlayerActor::Render(const CStateManager& mgr) const
{
    ...
    if (x314_beamModelData && !x314_beamModelData->IsNull() &&
        x64_modelData && !x64_modelData->IsNull())
    {
    ...
    if (phazonSuit)
    {
        ...
        g_Renderer->DrawPhazonSuitIndirectEffect(zeus::CColor(0.1f, 1.f), x338_phazonIndirectTexture,
                                                 zeus::CColor::skWhite, radius, 0.05f, offsetX, offsetY);
    }
}
