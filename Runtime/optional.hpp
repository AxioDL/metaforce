...
extern logvisor::Module OptionalLogModule;
...
# if OPTIONAL_HAS_MOVE_ACCESSORS == 1
  ...
  constexpr T const& value() const& {
    return initialized() ? contained_val() : (OptionalLogModule.report(logvisor::Fatal, "bad optional access"), contained_val());
  }
  ...
  OPTIONAL_MUTABLE_CONSTEXPR T&& value() && {
    if (!initialized()) OptionalLogModule.report(logvisor::Fatal, "bad optional access");
...
public:
  ...
  constexpr T& value() const {
    return ref ? *ref : (OptionalLogModule.report(logvisor::Fatal, "bad optional access"), *ref);
  }
